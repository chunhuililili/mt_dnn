    ...
    with open(out_path, "w", encoding="utf-8") as out_f:
        ...
        for row in rows:
            ...
            if data_format == DataFormat.PremiseOnly:
                ...
                for col in ["uid", "label", "text_a", "text_b"]:
                    if "\t" in str(row[col]):
                        import pdb; pdb.set_trace()
                out_f.write("%s\t%s\t%s\t%s\n" % (row["uid"], row["label"], row["text_a"], row["text_b"]))
            elif data_format == DataFormat.SimPairTest:
                for col in ["uid", "text_a", "text_b"]:
                    if "\t" in str(row[col]):
                        import pdb; pdb.set_trace()
                out_f.write("%s\t%s\t%s\n" % (row["uid"], row["text_a"], row["text_b"]))
            else:
